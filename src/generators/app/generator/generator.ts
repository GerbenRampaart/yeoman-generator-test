...
import { Answers, promptingImpl } from '../priorities/prompting';
...
export class TestGenerator extends Generator<GeneratorOptions> {
  ...
  private _answers: Answers | undefined = undefined;

  private get answers(): Answers {
    if (!this._answers) {
      throw new Error('Answers requested before initialized');
    }
  ...
  async prompting(): Promise<void> {
    this._answers = await promptingImpl(this);
  }
